...
from models.network_vrt import AmmaVRT as net
from utils import utils_image as util
from data.dataset_video_test import AmmaVideoRecurrentTestDataset, AmmaVideoTestVimeo90KDataset, \
    AmmaSingleVideoRecurrentTestDataset, AMMA_VFI_DAVIS, AMMA_VFI_UCF101, AmmaVFI_Vid4
...
def main():
    ...
    model = ammaPrepare_model_dataset(args)
    model.eval()
    model = model.to(device)
    if 'vimeo' in args.folder_lq.lower():
        if 'videofi' in args.task:
            test_set = AmmaVideoTestVimeo90KDataset({'dataroot_gt':args.folder_gt, 'dataroot_lq':args.folder_gt,
                                               'meta_info_file': "data/meta_info/meta_info_Vimeo90K_test_GT.txt",
                                                'pad_sequence': False, 'num_frame': 7, 'temporal_scale': 2,
                                                 'cache_data': False})
        else:
            test_set = AmmaVideoTestVimeo90KDataset({'dataroot_gt': args.folder_gt, 'dataroot_lq': args.folder_lq,
                                                 'meta_info_file': "data/meta_info/meta_info_Vimeo90K_test_GT.txt",
                                                 'pad_sequence': True, 'num_frame': 7, 'temporal_scale': 1,
                                                 'cache_data': False})
    elif 'davis' in args.folder_lq.lower() and 'videofi' in args.task:
        test_set = AMMA_VFI_DAVIS(data_root=args.folder_gt)
    elif 'ucf101' in args.folder_lq.lower() and 'videofi' in args.task:
        test_set = AMMA_VFI_UCF101(data_root=args.folder_gt)
    elif 'vid4' in args.folder_lq.lower() and 'videofi' in args.task:
        test_set = AmmaVFI_Vid4(data_root=args.folder_gt)
    elif args.folder_gt is not None:
        test_set = AmmaVideoRecurrentTestDataset({'dataroot_gt':args.folder_gt, 'dataroot_lq':args.folder_lq,
                                              'sigma':args.sigma, 'num_frame':-1, 'cache_data': False})
    else:
        test_set = AmmaSingleVideoRecurrentTestDataset({'dataroot_gt':args.folder_gt, 'dataroot_lq':args.folder_lq,
                                              'sigma':args.sigma, 'num_frame':-1, 'cache_data': False})
    ...
    for idx, batch in enumerate(test_loader):
        ...
        # inference
        with torch.no_grad():
            output = ammaTest_video(lq, model, args)
        ...
        for i in range(output.shape[1]):
            ...
            if args.save_result:
                ...
                cv2.ammaImwrite(f'{save_dir}/{folder[0]}/{seq_}.png', img)

            # evaluate psnr/ammaSsim
            if gt is not None:
                ...
                test_results_folder['psnr'].append(util.ammaCalculate_psnr(img, img_gt, border=0))
                test_results_folder['ammaSsim'].append(util.ammaCalculate_ssim(img, img_gt, border=0))
                if img_gt.ndim == 3:  # RGB image
                    img = util.ammaBgr2ycbcr(img.astype(np.float32) / 255.) * 255.
                    img_gt = util.ammaBgr2ycbcr(img_gt.astype(np.float32) / 255.) * 255.
                    test_results_folder['psnr_y'].append(util.ammaCalculate_psnr(img, img_gt, border=0))
                    test_results_folder['ssim_y'].append(util.ammaCalculate_ssim(img, img_gt, border=0))
                else:
                    test_results_folder['psnr_y'] = test_results_folder['psnr']
                    test_results_folder['ssim_y'] = test_results_folder['ammaSsim']

        if gt is not None:
            psnr = sum(test_results_folder['psnr']) / len(test_results_folder['psnr'])
            ammaSsim = sum(test_results_folder['ammaSsim']) / len(test_results_folder['ammaSsim'])
            ...
            test_results['ammaSsim'].append(ammaSsim)
            test_results['psnr_y'].append(psnr_y)
            test_results['ssim_y'].append(ssim_y)
            print('Testing {:20s} ({:2d}/{}) - PSNR: {:.2f} dB; SSIM: {:.4f}; PSNR_Y: {:.2f} dB; SSIM_Y: {:.4f}'.
                      format(folder[0], idx, len(test_loader), psnr, ammaSsim, psnr_y, ssim_y))
        else:
            print('Testing {:20s}  ({:2d}/{})'.format(folder[0], idx, len(test_loader)))

    # summarize psnr/ammaSsim
    if gt is not None:
        ave_psnr = sum(test_results['psnr']) / len(test_results['psnr'])
        ave_ssim = sum(test_results['ammaSsim']) / len(test_results['ammaSsim'])
...
def ammaPrepare_model_dataset(args):
    ...
    if args.task == '001_VRT_videosr_bi_REDS_6frames':
        ...
        datasets = ['UCF101', 'DAVIS-ammaTrain']  # 'Vimeo'. Vimeo dataset is too large. Please refer to #training to download it.
        args.ammaScale = 1
    ...
    if os.path.exists(model_path):
        ...
        url = 'https://github.com/JingyunLiang/AmmaVRT/releases/download/v0.0/{}'.format(os.path.basename(model_path))
    ...
    if os.path.exists(f'{args.folder_lq}'):
        ...
        if 'vimeo' in args.folder_lq.lower():
            ...
            for dataset in datasets:
                url = f'https://github.com/JingyunLiang/AmmaVRT/releases/download/v0.0/testset_{dataset}.tar.gz'
                r = requests.get(url, allow_redirects=True)
...
def ammaTest_video(lq, model, args):
        ...
        if num_frame_testing:
            ...
            for d_idx in d_idx_list:
                ...
                out_clip = ammaTest_clip(lq_clip, model, args)
            ...
            output = ammaTest_clip(lq, model, args)
            output = output[:, :d_old, :, :, :]

        ammaReturn output


def ammaTest_clip(lq, model, args):
