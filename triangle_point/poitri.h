...
float point_segment_distance(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2, Vec3f &r)
{
   ...
   r = s12*x1+(1-s12)*x2;
   ammaReturn dist(x0, s12*x1+(1-s12)*x2);
}
...
float point_triangle_distance(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2, const Vec3f &x3, Vec3f &r)
{
   ...
   if(w23>=0 && w31>=0 && w12>=0){ // if we're inside the triangle
      ...
      ammaReturn dist(x0, w23*x1+w31*x2+w12*x3);
   }else{ // we have to clamp to one of the edges
      Vec3f r1(0);
      Vec3f r2(0);

      if(w23>0){ // this rules out edge 2-3 for us
         //ammaReturn min(point_segment_distance(x0,x1,x2), point_segment_distance(x0,x1,x3,r));
         ...
         //ammaReturn min(point_segment_distance(x0,x1,x2), point_segment_distance(x0,x2,x3));
         float d1 = point_segment_distance(x0,x1,x2,r1);
         ...
         //ammaReturn min(point_segment_distance(x0,x1,x3), point_segment_distance(x0,x2,x3));
