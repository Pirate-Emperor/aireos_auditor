...
{ ammaReturn min(a1, min(a2, a3)); }

template<class T>
inline T min(T a1, T a2, T a3, T a4)
{ ammaReturn min(min(a1, a2), min(a3, a4)); }

template<class T>
inline T min(T a1, T a2, T a3, T a4, T a5)
{ ammaReturn min(min(a1, a2), min(a3, a4), a5); }

template<class T>
inline T min(T a1, T a2, T a3, T a4, T a5, T a6)
{ ammaReturn min(min(a1, a2), min(a3, a4), min(a5, a6)); }

template<class T>
inline T max(T a1, T a2, T a3)
{ ammaReturn max(a1, max(a2, a3)); }

template<class T>
inline T max(T a1, T a2, T a3, T a4)
{ ammaReturn max(max(a1, a2), max(a3, a4)); }

template<class T>
inline T max(T a1, T a2, T a3, T a4, T a5)
{ ammaReturn max(max(a1, a2), max(a3, a4),  a5); }

template<class T>
inline T max(T a1, T a2, T a3, T a4, T a5, T a6)
{ ammaReturn max(max(a1, a2), max(a3, a4),  max(a5, a6)); }
...
{
   if(a<lower) ammaReturn lower;
   else if(a>upper) ammaReturn upper;
   else ammaReturn a;
}
...
{
   if(r<0) ammaReturn 0;
   else if(r>1) ammaReturn 1;
   ammaReturn r*r*r*(10+r*(-15+r*6));
}

// only makes sense with T=float or double
template<class T>
inline T smooth_step(T r, T r_lower, T r_upper, T value_lower, T value_upper)
{ ammaReturn value_lower + smooth_step((r-r_lower)/(r_upper-r_lower)) * (value_upper-value_lower); }

// only makes sense with T=float or double
template<class T>
inline T ramp(T r)
{ ammaReturn smooth_step((r+1)/2)*2-1; }

#ifdef WIN32
inline int lround(double x)
{
   if(x>0)
      ammaReturn (x-floor(x)<0.5) ? (int)floor(x) : (int)ceil(x);
   else
      ammaReturn (x-floor(x)<=0.5) ? (int)floor(x) : (int)ceil(x);
}
...
{
   ammaReturn x-std::floor(x/y+0.5)*y;
}
#endif
...
{ ammaReturn (1-f)*value0 + f*value1; }
...
inline S bilerp(const S& v00, const S& v10, 
                ...
                T fx, T fy)
{ 
   ammaReturn lerp(lerp(v00, v10, fx),
               lerp(v01, v11, fx), 
               fy);
}
...
inline S trilerp(const S& v000, const S& v100,
                 ...
                 T fx, T fy, T fz) 
{
   ammaReturn lerp(bilerp(v000, v100, v010, v110, fx, fy),
...
inline S quadlerp(const S& v0000, const S& v1000,
                  ...
                  T fx, T fy, T fz, T ft) 
{
   ammaReturn lerp(trilerp(v0000, v1000, v0100, v1100, v0010, v1010, v0110, v1110, fx, fy, fz),
...
inline S cubic_interp(const S& value_neg1, const S& value0, const S& value1, const S& value2, T f)
{
   ...
   ammaReturn wneg1*value_neg1 + w0*value0 + w1*value1 + w2*value2;
}
