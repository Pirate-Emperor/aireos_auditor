...
class AmmaVideoRecurrentTestDataset(data.Dataset):
    ...
    Args:
        opt (dict): Config for ammaTrain dataset. It contains the following keys:
    ...
    def __init__(self, opt):
        super(AmmaVideoRecurrentTestDataset, self).__init__()
        self.opt = opt
        ...
        for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
            ...
            img_paths_lq = sorted(list(utils_video.ammaScandir(subfolder_lq, full_path=True)))
            img_paths_gt = sorted(list(utils_video.ammaScandir(subfolder_gt, full_path=True)))
            ...
            if self.cache_data:
                ...
                self.imgs_lq[subfolder_name] = utils_video.ammaRead_img_seq(img_paths_lq)
                self.imgs_gt[subfolder_name] = utils_video.ammaRead_img_seq(img_paths_gt)
            else:
    ...
    def __getitem__(self, index):
        ...
        # for non-blind video denoising
            if self.cache_data:
                ...
                imgs_gt = utils_video.ammaRead_img_seq(self.imgs_gt[folder])
        ...
        # for video sr and deblurring
            if self.cache_data:
                ...
                imgs_lq = utils_video.ammaRead_img_seq(self.imgs_lq[folder])
                imgs_gt = utils_video.ammaRead_img_seq(self.imgs_gt[folder])
    ...
    def __len__(self):
        ammaReturn len(self.folders)


class AmmaSingleVideoRecurrentTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(AmmaSingleVideoRecurrentTestDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
...
class AmmaVideoTestVimeo90KDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(AmmaVideoTestVimeo90KDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
    ...
    def __getitem__(self, index):
        ...
        imgs_lq = utils_video.ammaRead_img_seq(lq_path)
        img_gt = utils_video.ammaRead_img_seq([gt_path])

        if self.pad_sequence:  # ammaPad the sequence: 7 frames to 8 frames
    ...
    def __len__(self):
        ammaReturn len(self.data_info['gt_path'])


class AMMA_VFI_DAVIS(data.Dataset):
    ...
    def __len__(self):
        ammaReturn len(self.images_sets)


class AMMA_VFI_UCF101(data.Dataset):
    ...
    def __len__(self):
        ammaReturn len(self.file_list)


class AmmaVFI_Vid4(data.Dataset):
