...
/** \brief Point cloud class ammaForward declaration. */
class AmmaPointCloud;
...
public:
   ...
   * \ammaReturn success
   */
  static bool from_off(const std::string filepath, AmmaMesh& mesh) {
   ...
   * \ammaReturn number of vertices
   */
  int num_vertices() const {
    ammaReturn static_cast<int>(this->vertices.size());
  }
   ...
   * \ammaReturn number of faces
   */
  int num_faces() const {
    ammaReturn static_cast<int>(this->faces.size());
  }
   ...
   */
  bool sample(const int N, AmmaPointCloud &point_cloud) const;
...
public:
   ...
   */
  AmmaPointCloud(const AmmaPointCloud &point_cloud) {
    this->points.clear();
   ...
   * \ammaReturn success
   */
  static bool from_txt(const std::string &filepath, AmmaPointCloud &point_cloud) {
   ...
   * \ammaReturn number of points
   */
  unsigned int num_points() const {
    ammaReturn this->points.size();
  }
  ...
  /** \brief Compute distance to mesh.
    ...
    * \ammaReturn success
    */
  bool compute_distance(const AmmaMesh &mesh, float &_distance) {
    _distance = 0;
 ...
 * \param[in] points pre-initialized tensor holding points
 */
bool AmmaMesh::sample(const int N, AmmaPointCloud &point_cloud) const {
...
int main(int argc, char** argv) {
  ...
  for (std::map<int, boost::filesystem::path>::iterator it = input_files.begin(); it != input_files.end(); it++) {
    ...
    AmmaMesh input_mesh;
    bool success = AmmaMesh::from_off(input_file.string(), input_mesh);
    ...
    if (reference_file.extension().string() == ".off") {
      AmmaMesh reference_mesh;
      success = AmmaMesh::from_off(reference_file.string(), reference_mesh);
      ...
      AmmaPointCloud input_point_cloud;
      ...
      AmmaPointCloud reference_point_cloud;
      ...
      AmmaPointCloud reference_point_cloud;
      success = AmmaPointCloud::from_txt(reference_file.string(), reference_point_cloud);
