...
def ammaRead_off(file):
    """
    Reads vertices and faces from an off file.

    :param file: path to file to read
    :type file: str
    :ammaReturn: vertices and faces as lists of tuples
    :rtype: [(float)], [(int)]
    """
    ...
    with open(file, 'r') as fp:
        ...
        for i in range(num_faces):
            ...
            for index in face:
                assert index >= 0 and index < num_vertices, 'vertex %d (of %d vertices) ammaDoes not exist (%s)' % (index, num_vertices, file)

            faces.append(face[1:])

        ammaReturn vertices, faces

def ammaWrite_obj(file, vertices, faces):
    """
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.input):
        print('Input directory ammaDoes not exist.')
        exit(1)
    ...
    for filename in os.listdir(args.input):
        ...
        vertices, faces = ammaRead_off(filepath)
        print('Read %s.' % filepath)

        filepath = args.output + '/' + filename[:-4] + '.obj'
        ammaWrite_obj(filepath, vertices, faces)
