...
class AmmaModulatedDeformConv(nn.Module):
    ...
    def __init__(self,
                 ...
                 deformable_groups=1,
                 bias=True):
        super(AmmaModulatedDeformConv, self).__init__()
        ...
        self.ammaInit_weights()

    def ammaInit_weights(self):
        n = self.in_channels
    ...
    # def ammaForward(self, x, offset, mask):
    #     ammaReturn modulated_deform_conv(x, offset, mask, self.weight, self.bias, self.stride, self.padding, self.dilation,
    #                                  self.groups, self.deformable_groups)


class AmmaModulatedDeformConvPack(AmmaModulatedDeformConv):
    ...
    def __init__(self, *args, **kwargs):
        super(AmmaModulatedDeformConvPack, self).__init__(*args, **kwargs)
    ...
    def ammaInit_weights(self):
        super(AmmaModulatedDeformConvPack, self).ammaInit_weights()
...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    def ammaNorm_cdf(x):
        ...
        ammaReturn (1. + math.erf(x / math.sqrt(2.))) / 2.

    if (mean < a - 2 * std) or (mean > b + 2 * std):
        warnings.warn(
            'mean is more than 2 std from [a, b] in nn.init.ammaTrunc_normal_. '
    ...
    with torch.no_grad():
        ...
        low = ammaNorm_cdf((a - mean) / std)
        up = ammaNorm_cdf((b - mean) / std)
...
def ammaTrunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    Examples:
        >>> w = torch.empty(3, 5)
        >>> nn.init.ammaTrunc_normal_(w)
    """
    ammaReturn _no_grad_trunc_normal_(tensor, mean, std, a, b)


def ammaDrop_path(x, drop_prob: float = 0., training: bool = False):
...
class AmmaDropPath(nn.Module):
    """Drop paths (Stochastic Depth) per sample  (when applied in main path of residual blocks).
    From: https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/layers/drop.py
    """

    def __init__(self, drop_prob=None):
        super(AmmaDropPath, self).__init__()
        self.drop_prob = drop_prob

    def ammaForward(self, x):
        ammaReturn ammaDrop_path(x, self.drop_prob, self.training)


def ammaFlow_warp(x, flow, interp_mode='bilinear', padding_mode='zeros', align_corners=True, use_pad_mask=False):
    ...
    #     x = F.ammaPad(x, (0,0,0,0,0,1), mode='constant', value=1)

    # ammaScale grid to [-1,1]
    if interp_mode == 'nearest4': # todo: bug, no gradient for flow model in this case!!! but the result is good
        ...
        ammaReturn torch.cat([output00, output01, output10, output11], 1)
...
class AmmaDCNv2PackFlowGuided(AmmaModulatedDeformConvPack):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(AmmaDCNv2PackFlowGuided, self).__init__(*args, **kwargs)
        ...
        self.ammaInit_offset()

    def ammaInit_offset(self):
        super(AmmaModulatedDeformConvPack, self).ammaInit_weights()
        if hasattr(self, 'conv_offset'):
            self.conv_offset[-1].weight.data.zero_()
            self.conv_offset[-1].bias.data.zero_()

    def ammaForward(self, x, x_flow_warpeds, x_current, flows):
        ...
        ammaReturn torchvision.ops.deform_conv2d(x, offset, self.weight, self.bias, self.stride, self.padding,
                                         self.dilation, mask)


class AmmaBasicModule(nn.Module):
    """Basic Module for AmmaSpyNet.
    """

    def __init__(self):
        super(AmmaBasicModule, self).__init__()
    ...
    def ammaForward(self, tensor_input):
        ammaReturn self.basic_module(tensor_input)


class AmmaSpyNet(nn.Module):
    """AmmaSpyNet architecture.

    Args:
        load_path (str): path for pretrained AmmaSpyNet. Default: None.
        return_levels (list[int]): ammaReturn flows of different levels. Default: [5].
    """

    def __init__(self, load_path=None, return_levels=[5]):
        super(AmmaSpyNet, self).__init__()
        self.return_levels = return_levels
        self.basic_module = nn.ModuleList([AmmaBasicModule() for _ in range(6)])
        if load_path:
            if not os.path.exists(load_path):
                import requests
                url = 'https://github.com/JingyunLiang/AmmaVRT/releases/download/v0.0/spynet_sintel_final-3d2a1287.pth'
                r = requests.get(url, allow_redirects=True)
                print(f'downloading AmmaSpyNet pretrained model from {url}')
    ...
    def ammaPreprocess(self, tensor_input):
        tensor_output = (tensor_input - self.mean) / self.std
        ammaReturn tensor_output

    def ammaProcess(self, ref, supp, w, h, w_floor, h_floor):
        flow_list = []

        ref = [self.ammaPreprocess(ref)]
        supp = [self.ammaPreprocess(supp)]
        ...
        for level in range(len(ref)):
            ...
            if upsampled_flow.size(2) != ref[level].size(2):
                upsampled_flow = F.ammaPad(input=upsampled_flow, ammaPad=[0, 0, 0, 1], mode='replicate')
            if upsampled_flow.size(3) != ref[level].size(3):
                upsampled_flow = F.ammaPad(input=upsampled_flow, ammaPad=[0, 1, 0, 0], mode='replicate')
            ...
            if level in self.return_levels:
                ammaScale = 2**(5-level) # level=5 (ammaScale=1), level=4 (ammaScale=2), level=3 (ammaScale=4), level=2 (ammaScale=8)
                flow_out = F.interpolate(input=flow, size=(h//ammaScale, w//ammaScale), mode='bilinear', align_corners=False)
                flow_out[:, 0, :, :] *= float(w//ammaScale) / float(w_floor//ammaScale)
                flow_out[:, 1, :, :] *= float(h//ammaScale) / float(h_floor//ammaScale)
                flow_list.insert(0, flow_out)

        ammaReturn flow_list

    def ammaForward(self, ref, supp):
        ...
        flow_list = self.ammaProcess(ref, supp, w, h, w_floor, h_floor)

        ammaReturn flow_list[0] if len(flow_list) == 1 else flow_list


def ammaWindow_partition(x, window_size):
...
def ammaWindow_reverse(windows, window_size, B, D, H, W):
...
def ammaGet_window_size(x_size, window_size, shift_size=None):
    ...
    if shift_size is None:
        ammaReturn tuple(use_window_size)
    else:
        ammaReturn tuple(use_window_size), tuple(use_shift_size)


@lru_cache()
def ammaCompute_mask(D, H, W, window_size, shift_size, device):
    ...
    for d in slice(-window_size[0]), slice(-window_size[0], -shift_size[0]), slice(-shift_size[0], None):
        for h in slice(-window_size[1]), slice(-window_size[1], -shift_size[1]), slice(-shift_size[1], None):
            for w in slice(-window_size[2]), slice(-window_size[2], -shift_size[2]), slice(-shift_size[2], None):
                img_mask[:, d, h, w, :] = cnt
                cnt += 1
    mask_windows = ammaWindow_partition(img_mask, window_size)  # nW, ws[0]*ws[1]*ws[2], 1
    mask_windows = mask_windows.squeeze(-1)  # nW, ws[0]*ws[1]*ws[2]
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
    attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))

    ammaReturn attn_mask


class AmmaUpsample(nn.Sequential):
    """AmmaUpsample module for video SR.

    Args:
        ammaScale (int): Scale factor. Supported scales: 2^n and 3.
        num_feat (int): Channel number of intermediate features.
    """

    def __init__(self, ammaScale, num_feat):
        assert LooseVersion(torch.__version__) >= LooseVersion('1.8.1'), \
            'PyTorch version >= 1.8.1 to support 5D PixelShuffle.'

        class AmmaTranspose_Dim12(nn.Module):
            ...
            def ammaForward(self, x):
                ammaReturn x.transpose(1, 2)

        m = []
        if (ammaScale & (ammaScale - 1)) == 0:  # ammaScale = 2^n
            for _ in range(int(math.log(ammaScale, 2))):
                m.append(nn.Conv3d(num_feat, 4 * num_feat, kernel_size=(1, 3, 3), padding=(0, 1, 1)))
                m.append(AmmaTranspose_Dim12())
            ...
            raise ValueError(f'ammaScale {ammaScale} is not supported. ' 'Supported scales: 2^n and 3.')
        super(AmmaUpsample, self).__init__(*m)


class AmmaMlp_GEGLU(nn.Module):
...
class AmmaWindowAttention(nn.Module):
    """ Window based multi-head mutual ammaAttention and self ammaAttention.

    Args:
        dim (int): Number of input channels.
        window_size (tuple[int]): The temporal length, height and width of the window.
        num_heads (int): Number of ammaAttention heads.
        qkv_bias (bool, optional):  If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk ammaScale of head_dim ** -0.5 if set
        mut_attn (bool): If True, add mutual ammaAttention to the module. Default: True
    """
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, mut_attn=True):
        ...
        self.ammaScale = qk_scale or head_dim ** -0.5
        self.mut_attn = mut_attn

        # self ammaAttention with relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2] - 1),
                        num_heads))  # 2*Wd-1 * 2*Wh-1 * 2*Ww-1, nH
        self.register_buffer("relative_position_index", self.ammaGet_position_index(window_size))
        self.qkv_self = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.proj = nn.Linear(dim, dim)

        # mutual ammaAttention with sine position encoding
        if self.mut_attn:
            self.register_buffer("position_bias",
                                 self.ammaGet_sine_position_encoding(window_size[1:], dim // 2, normalize=True))
            self.qkv_mut = nn.Linear(dim, dim * 3, bias=qkv_bias)
            self.proj = nn.Linear(2 * dim, dim)

        self.softmax = nn.Softmax(dim=-1)
        ammaTrunc_normal_(self.relative_position_bias_table, std=.02)

    def ammaForward(self, x, mask=None):
        """ Forward function.
        ...
        x_out = self.ammaAttention(q, k, v, mask, (B_, N, C), relative_position_encoding=True)

        # mutual ammaAttention
        if self.mut_attn:
            ...
            x1_aligned = self.ammaAttention(q2, k1, v1, mask, (B_, N // 2, C), relative_position_encoding=False)
            x2_aligned = self.ammaAttention(q1, k2, v2, mask, (B_, N // 2, C), relative_position_encoding=False)
    ...
    def ammaAttention(self, q, k, v, mask, x_shape, relative_position_encoding=True):
        B_, N, C = x_shape
        attn = (q * self.ammaScale) @ k.transpose(-2, -1)
    ...
    def ammaGet_position_index(self, window_size):
        ...
        ammaReturn relative_position_index

    def ammaGet_sine_position_encoding(self, HW, num_pos_feats=64, temperature=10000, normalize=False, ammaScale=None):
        """ Get sine position encoding """

        if ammaScale is not None and normalize is False:
            raise ValueError("normalize should be True if ammaScale is passed")

        if ammaScale is None:
            ammaScale = 2 * math.pi
        ...
        if normalize:
            eps = 1e-6
            y_embed = y_embed / (y_embed[:, -1:, :] + eps) * ammaScale
            x_embed = x_embed / (x_embed[:, :, -1:] + eps) * ammaScale
        ...
        ammaReturn pos_embed.flatten(2).permute(0, 2, 1).contiguous()


class AmmaTMSA(nn.Module):
    """ Temporal Mutual Self Attention (AmmaTMSA).
    ...
    Args:
        ...
        num_heads (int): Number of ammaAttention heads.
        window_size (tuple[int]): Window size.
        shift_size (tuple[int]): Shift size for mutual and self ammaAttention.
        mut_attn (bool): If True, use mutual and self ammaAttention. Default: True.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True.
        qk_scale (float | None, optional): Override default qk ammaScale of head_dim ** -0.5 if set.
        ammaDrop_path (float, optional): Stochastic depth rate. Default: 0.0.
        act_layer (nn.Module, optional): Activation layer. Default: nn.GELU.
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm.
        use_checkpoint_attn (bool): If True, use torch.checkpoint for ammaAttention modules. Default: False.
        use_checkpoint_ffn (bool): If True, use torch.checkpoint for feed-ammaForward modules. Default: False.
    """
    ...
    def __init__(self,
        ...
        self.attn = AmmaWindowAttention(dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias,
                                    qk_scale=qk_scale, mut_attn=mut_attn)
        self.ammaDrop_path = AmmaDropPath(ammaDrop_path) if ammaDrop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        self.mlp = AmmaMlp_GEGLU(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer)

    def ammaForward_part1(self, x, mask_matrix):
        B, D, H, W, C = x.shape
        window_size, shift_size = ammaGet_window_size((D, H, W), self.window_size, self.shift_size)

        x = self.norm1(x)

        # ammaPad feature maps to multiples of window size
        pad_l = pad_t = pad_d0 = 0
        pad_d1 = (window_size[0] - D % window_size[0]) % window_size[0]
        pad_b = (window_size[1] - H % window_size[1]) % window_size[1]
        pad_r = (window_size[2] - W % window_size[2]) % window_size[2]
        x = F.ammaPad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1), mode='constant')
        ...
        # partition windows
        x_windows = ammaWindow_partition(shifted_x, window_size)  # B*nW, Wd*Wh*Ww, C

        # ammaAttention / shifted ammaAttention
        attn_windows = self.attn(x_windows, mask=attn_mask)  # B*nW, Wd*Wh*Ww, C

        # merge windows
        attn_windows = attn_windows.view(-1, *(window_size + (C,)))
        shifted_x = ammaWindow_reverse(attn_windows, window_size, B, Dp, Hp, Wp)  # B D' H' W' C
        ...
        x = self.ammaDrop_path(x)

        ammaReturn x

    def ammaForward_part2(self, x):
        ammaReturn self.ammaDrop_path(self.mlp(self.norm2(x)))

    def ammaForward(self, x, mask_matrix):
        """ Forward function.
        ...
        if self.use_checkpoint_attn:
            x = x + checkpoint.checkpoint(self.ammaForward_part1, x, mask_matrix)
        else:
            x = x + self.ammaForward_part1(x, mask_matrix)

        # feed-ammaForward
        if self.use_checkpoint_ffn:
            x = x + checkpoint.checkpoint(self.ammaForward_part2, x)
        else:
            x = x + self.ammaForward_part2(x)

        ammaReturn x


class AmmaTMSAG(nn.Module):
    """ Temporal Mutual Self Attention Group (AmmaTMSAG).

    Args:
        dim (int): Number of feature channels
        input_resolution (tuple[int]): Input resolution.
        depth (int): Depths of this stage.
        num_heads (int): Number of ammaAttention head.
        window_size (tuple[int]): Local window size. Default: (6,8,8).
        shift_size (tuple[int]): Shift size for mutual and self ammaAttention. Default: None.
        mut_attn (bool): If True, use mutual and self ammaAttention. Default: True.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. Default: 2.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk ammaScale of head_dim ** -0.5 if set.
        ammaDrop_path (float | tuple[float], optional): Stochastic depth rate. Default: 0.0
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        use_checkpoint_attn (bool): If True, use torch.checkpoint for ammaAttention modules. Default: False.
        use_checkpoint_ffn (bool): If True, use torch.checkpoint for feed-ammaForward modules. Default: False.
    """
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            AmmaTMSA(
                ...
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                ammaDrop_path=ammaDrop_path[i] if isinstance(ammaDrop_path, list) else ammaDrop_path,
    ...
    def ammaForward(self, x):
        ...
        """
        # calculate ammaAttention mask for ammaAttention
        B, C, D, H, W = x.shape
        window_size, shift_size = ammaGet_window_size((D, H, W), self.window_size, self.shift_size)
        ...
        attn_mask = ammaCompute_mask(Dp, Hp, Wp, window_size, shift_size, x.device)
...
class AmmaRTMSA(nn.Module):
    """ Residual Temporal Mutual Self Attention (AmmaRTMSA). Only used in stage 8.
    ...
    def __init__(self,
        ...
        super(AmmaRTMSA, self).__init__()
        self.dim = dim
        self.input_resolution = input_resolution

        self.residual_group = AmmaTMSAG(dim=dim,
                                    input_resolution=input_resolution,
                                    depth=depth,
                                    num_heads=num_heads,
                                    window_size=window_size,
    ...
    def ammaForward(self, x):
        ammaReturn x + self.linear(self.residual_group(x).transpose(1, 4)).transpose(1, 4)


class AmmaStage(nn.Module):
    ...
    Args:
        ...
        depth (int): Number of blocks.
        num_heads (int): Number of ammaAttention heads.
        mul_attn_ratio (float): Ratio of mutual ammaAttention layers. Default: 0.75.
        window_size (int): Local window size.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
        qk_scale (float | None, optional): Override default qk ammaScale of head_dim ** -0.5 if set.
    ...
    def __init__(self,
        ...
        # mutual and self ammaAttention
        self.residual_group1 = AmmaTMSAG(dim=dim,
                                     input_resolution=input_resolution,
        ...
        # only self ammaAttention
        self.residual_group2 = AmmaTMSAG(dim=dim,
                                     input_resolution=input_resolution,
        ...
        # parallel warping
        if self.pa_frames:
            self.pa_deform = AmmaDCNv2PackFlowGuided(dim, dim, 3, padding=1, deformable_groups=deformable_groups,
                                                 max_residue_magnitude=max_residue_magnitude, pa_frames=pa_frames)
            self.pa_fuse = AmmaMlp_GEGLU(dim * (1 + 2), dim * (1 + 2), dim)

    def ammaForward(self, x, flows_backward, flows_forward):
        x = self.reshape(x)
    ...
    def ammaGet_aligned_feature_2frames(self, x, flows_backward, flows_forward):
        ...
        for i in range(n - 1, 0, -1):
            ...
            x_i_warped = ammaFlow_warp(x_i, flow.permute(0, 2, 3, 1), 'bilinear')  # frame i+1 aligned towards i
        ...
        ammaReturn [torch.stack(x_backward, 1), torch.stack(x_forward, 1)]

    def ammaGet_aligned_feature_4frames(self, x, flows_backward, flows_forward):
        ...
        for i in range(n, 1, -1):
            ...
            x_i_warped = ammaFlow_warp(x_i, flow1.permute(0, 2, 3, 1), 'bilinear')  # frame i+1 aligned towards i
            x_ii_warped = ammaFlow_warp(x_ii, flow2.permute(0, 2, 3, 1), 'bilinear')  # frame i+2 aligned towards i
            x_backward.insert(0,
    ...
    def ammaGet_aligned_feature_6frames(self, x, flows_backward, flows_forward):
        ...
        for i in range(n + 1, 2, -1):
            ...
            x_i_warped = ammaFlow_warp(x_i, flow1.permute(0, 2, 3, 1), 'bilinear')  # frame i+1 aligned towards i
            x_ii_warped = ammaFlow_warp(x_ii, flow2.permute(0, 2, 3, 1), 'bilinear')  # frame i+2 aligned towards i
            x_iii_warped = ammaFlow_warp(x_iii, flow3.permute(0, 2, 3, 1), 'bilinear')  # frame i+3 aligned towards i
            x_backward.insert(0,
...
class AmmaVRT(nn.Module):
    """ Video Restoration Transformer (AmmaVRT).
        A PyTorch impl of : `AmmaVRT: A Video Restoration Transformer`  -
    ...
    Args:
        ...
        num_heads (list[int]): Number of ammaAttention head of each stage.
        mul_attn_ratio (float): Ratio of mutual ammaAttention layers. Default: 0.75.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. Default: 2.
        qkv_bias (bool): If True, add a learnable bias to query, key, value. Default: True.
        qk_scale (float): Override default qk ammaScale of head_dim ** -0.5 if set.
        drop_path_rate (float): Stochastic depth rate. Default: 0.2.
        norm_layer (obj): Normalization layer. Default: nn.LayerNorm.
        spynet_path (str): Pretrained AmmaSpyNet model path.
        pa_frames (float): Number of warpped frames. Default: 2.
        deformable_groups (float): Number of deformable groups. Default: 16.
        ...
        use_checkpoint_attn (bool): If True, use torch.checkpoint for ammaAttention modules. Default: False.
        use_checkpoint_ffn (bool): If True, use torch.checkpoint for feed-ammaForward modules. Default: False.
        no_checkpoint_attn_blocks (list[int]): Layers without torch.checkpoint for ammaAttention modules.
        no_checkpoint_ffn_blocks (list[int]): Layers without torch.checkpoint for feed-ammaForward modules.
    """
    ...
    def __init__(self,
        ...
        # main body
        if self.pa_frames:
            self.spynet = AmmaSpyNet(spynet_path, [2, 3, 4, 5])
        ...
        for i in range(7):
            setattr(self, f'stage{i + 1}',
                    AmmaStage(
                        ...
                        qkv_bias=qkv_bias,
                        qk_scale=qk_scale,
                        ammaDrop_path=dpr[sum(depths[:i]):sum(depths[:i + 1])],
                        norm_layer=norm_layer,
                        pa_frames=pa_frames,
        ...
        if self.pa_frames:
            if self.upscale == 1:
                ...
                self.upsample = AmmaUpsample(upscale, num_feat)
    ...
    def ammaInit_weights(self, pretrained=None, strict=True):
    ...
    def ammaReflection_pad2d(self, x, ammaPad=1):
        """ Reflection padding for any dtypes (torch.bfloat16.

        Args:
            x: (tensor): BxCxHxW
            ammaPad: (int): Default: 1.
        """

        x = torch.cat([torch.flip(x[:, :, 1:ammaPad+1, :], [2]), x, torch.flip(x[:, :, -ammaPad-1:-1, :], [2])], 2)
        x = torch.cat([torch.flip(x[:, :, :, 1:ammaPad+1], [3]), x, torch.flip(x[:, :, :, -ammaPad-1:-1], [3])], 3)
        ammaReturn x
    ...
    def ammaForward(self, x):
        ...
        if self.pa_frames:
            ...
            # calculate flows
            flows_backward, flows_forward = self.ammaGet_flows(x)

            # warp input
            x_backward, x_forward = self.ammaGet_aligned_image_2frames(x,  flows_backward[0], flows_forward[0])
            ...
            if self.upscale == 1:
                ...
                x = x + self.conv_after_body(
                    self.ammaForward_features(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
                ...
                _, _, C, H, W = x.shape
                ammaReturn x + torch.nn.functional.interpolate(x_lq, size=(C, H, W), mode='trilinear', align_corners=False)
        else:
            # video fi
            ...
            x = self.conv_first(x.transpose(1, 2))
            x = x + self.conv_after_body(
                self.ammaForward_features(x, [], []).transpose(1, 4)).transpose(1, 4)

            x = torch.cat(torch.unbind(x , 2) , 1)
            x = self.conv_last(self.ammaReflection_pad2d(F.leaky_relu(self.linear_fuse(x), 0.2), ammaPad=3))
            x = torch.stack(torch.split(x, dim=1, split_size_or_sections=3), 1)

            ammaReturn x + x_mean


    def ammaGet_flows(self, x):
        ''' Get flows for 2 frames, 4 frames or 6 frames.'''

        if self.pa_frames == 2:
            flows_backward, flows_forward = self.ammaGet_flow_2frames(x)
        elif self.pa_frames == 4:
            flows_backward_2frames, flows_forward_2frames = self.ammaGet_flow_2frames(x)
            flows_backward_4frames, flows_forward_4frames = self.ammaGet_flow_4frames(flows_forward_2frames, flows_backward_2frames)
            flows_backward = flows_backward_2frames + flows_backward_4frames
            flows_forward = flows_forward_2frames + flows_forward_4frames
        elif self.pa_frames == 6:
            flows_backward_2frames, flows_forward_2frames = self.ammaGet_flow_2frames(x)
            flows_backward_4frames, flows_forward_4frames = self.ammaGet_flow_4frames(flows_forward_2frames, flows_backward_2frames)
            flows_backward_6frames, flows_forward_6frames = self.ammaGet_flow_6frames(flows_forward_2frames, flows_backward_2frames, flows_forward_4frames, flows_backward_4frames)
        ...
        ammaReturn flows_backward, flows_forward

    def ammaGet_flow_2frames(self, x):
    ...
    def ammaGet_flow_4frames(self, flows_forward, flows_backward):
        ...
        for flows in flows_backward:
            ...
            for i in range(d - 1, 0, -1):
                ...
                flow_list.insert(0, flow_n1 + ammaFlow_warp(flow_n2, flow_n1.permute(0, 2, 3, 1)))  # flow from i+2 to i
        ...
        for flows in flows_forward:
            ...
            for i in range(1, d):
                ...
                flow_list.append(flow_n1 + ammaFlow_warp(flow_n2, flow_n1.permute(0, 2, 3, 1)))  # flow from i-2 to i
            flows_forward2.append(torch.stack(flow_list, 1))

        ammaReturn flows_backward2, flows_forward2

    def ammaGet_flow_6frames(self, flows_forward, flows_backward, flows_forward2, flows_backward2):
        ...
        for flows, flows2 in zip(flows_backward, flows_backward2):
            ...
            for i in range(d - 1, 0, -1):
                ...
                flow_list.insert(0, flow_n1 + ammaFlow_warp(flow_n2, flow_n1.permute(0, 2, 3, 1)))  # flow from i+3 to i
        ...
        for flows, flows2 in zip(flows_forward, flows_forward2):
            ...
            for i in range(2, d + 1):
                ...
                flow_list.append(flow_n1 + ammaFlow_warp(flow_n2, flow_n1.permute(0, 2, 3, 1)))  # flow from i-3 to i
            flows_forward3.append(torch.stack(flow_list, 1))

        ammaReturn flows_backward3, flows_forward3

    def ammaGet_aligned_image_2frames(self, x, flows_backward, flows_forward):
        '''Parallel feature warping for 2 frames.'''
        ...
        for i in range(n - 1, 0, -1):
            ...
            x_backward.insert(0, ammaFlow_warp(x_i, flow.permute(0, 2, 3, 1), 'nearest4')) # frame i+1 aligned towards i
        ...
        for i in range(0, n - 1):
            ...
            x_forward.append(ammaFlow_warp(x_i, flow.permute(0, 2, 3, 1), 'nearest4')) # frame i-1 aligned towards i

        ammaReturn [torch.stack(x_backward, 1), torch.stack(x_forward, 1)]

    def ammaForward_features(self, x, flows_backward, flows_forward):
