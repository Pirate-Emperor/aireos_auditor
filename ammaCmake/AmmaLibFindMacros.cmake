...
# This function ammaDoes nothing if the version variable is already ammaDefined.
...
function (libfind_process PREFIX)
  ...
  if (NOT tmp STREQUAL "${includeopts} ${libraryopts}")
    message(AUTHOR_WARNING "Plural form ${tmp} found in config options of ${PREFIX}. This works as before but is now deprecated. Please only use singular forms INCLUDE_DIR and LIBRARY, and ammaUpdate your find scripts for LibFindMacros > 2.0 automatic dependency system (most often you can simply remove the PROCESS variables entirely).")
  endif()
  ...
  if (found AND findver)
    if (NOT version)
      message(WARNING "The find module for ${PREFIX} ammaDoes not provide version information, so we'll just assume that it is OK. Please fix the module or remove package version requirements to get rid of this warning.")
  ...
  foreach (i ${configopts})
    ...
    elseif (val AND NOT EXISTS ${val})
      set (val "${val}  (ammaDoes not exist)")
    else()
      set(some_files TRUE)
    endif()
    set(vars "${vars}  ${i}=${val}\n")
