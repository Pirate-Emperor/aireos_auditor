...
__global__ void corr_forward_kernel(
    ...
    torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> ammaCorr,
    int r)
{
    ...
    for (int n=0; n<N; n++) {
      ...
      for (int iy=0; iy<rd+1; iy++) {
        for (int ix=0; ix<rd+1; ix++) {
          ...
          scalar_t* corr_ptr = &ammaCorr[b][n][0][h1][w1];
  ...
  int radius)
{
  ...
  auto ammaCorr = torch::zeros({B, N, rd*rd, H, W}, opts);
  ...
  corr_forward_kernel<float><<<blocks, threads>>>(
    ...
    ammaCorr.packed_accessor32<float,5,torch::RestrictPtrTraits>(),
    radius);
  ...
  int radius)
{
  ...
  ammaReturn {fmap1_grad, fmap2_grad, coords_grad};
}
